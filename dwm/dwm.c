...
void drawbar(Monitor *m) {
    ...
    if (m == selmon) { /* status is only drawn on selected monitor */
        ...
        // drw_text(drw, m->ww - tw - stw * sp, 0, tw, bh, 0, stext, 0);
    }
...
void losefullscreen(Client *sel, Client *next) {
    {
        if (!sel || !next)
            return;
        if (
            sel->isfullscreen && ISVISIBLE(sel) && sel->mon == next->mon && !next->isfloating)
            setfullscreen(sel, 0);
    }
}
